...
const ALL_TAGS = [
  "#RoadIssue", "#Garbage", "#WaterProblem", "#Electricity", "#Drainage",
  "#PublicIssue", "#PeopleVoice", "#Urgent", "#Chennai", "#Nagercoil",
];
...
const TEXTS = {
  en: {
    ...
    legalText: "Submitting false, misleading or irrelevant information / photographs constitutes a serious offence. Government authorities may initiate legal action under Section 177 IPC and other applicable laws. Misuse of this platform may result in permanent account suspension.",
    ...
    acceptTermsDesc: "I understand that providing false information may result in legal action and account suspension.",
  ...
  ta: {
    ...
    legalText: "பொய், தவறான அல்லது தொடர்பில்லாத தகவல்/புகைப்படங்களை சமர்ப்பிப்பது கடுமையான குற்றமாகும். அரசாங்க அதிகாரிகள் பிரிவு 177 IPC மற்றும் பிற பொருந்தக்கூடிய சட்டங்களின் கீழ் சட்ட நடவடிக்கை தொடுக்கலாம். இந்த மேடையின் தவறான பயன்பாடு நிரந்தர கணக்கு நிறுத்தத்திற்கு வழிவகுக்கும்.",
    ...
    acceptTermsDesc: "பொய்த் தகவலை வழங்குவது சட்ட நடவடிக்கை மற்றும் கணக்கு நிறுத்தத்திற்கு வழிவகுக்கும் என்பதை நான் புரிந்துகொள்கிறேன்.",
    ...
    minPhotosRequired: "{department} க்கு குறைந்தபட்சம் {minImages} புகைப்பட(ங்கள்) தேவை",
...
const PostIssue = () => {
  ...
  useEffect(() => {
    if (department) {
      ...
      setValidationMessage(t("readyToCapture", { department: getDepartmentName(department) }));
    }
  }, [department]);
  ...
  const getDepartmentDetails = (id) => DEPARTMENTS.find(d => d.id === id) || DEPARTMENTS[5];
  ...
  const takePhoto = async () => {
    ...
    try {
      ...
      if (location?.address) {
        ...
        if (!district) {
          const found = DISTRICTS.find(d => location.address.toLowerCase().includes(d.toLowerCase()));
  ...
  const handleHashtagChange = (e) => {
    ...
    if (last?.startsWith("#") && last.length > 2) {
      setSuggestions(
        ALL_TAGS.filter(t => t.toLowerCase().includes(last.toLowerCase()) && !value.includes(t))
      );
    } else {
      setSuggestions([]);
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (!district || !area.trim() || !department || !reason || !agree || images.length === 0 ||
        !descEn.trim() || !descTa.trim()) {
      return setError(t("fillAllFields"));
    }

    if (images.length < dept.minImages) {
      return setError(t("minPhotosRequired", { minImages: dept.minImages, department: dept.name }));
    }
  ...
  const handleVerificationSubmit = async () => {
    // Check only the required checkboxes
     ...
     <div className="w-full max-w-[700px] mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 pb-24 sm:pb-32">
        ...
        {error && (
          <div className="mb-4 sm:mb-6 bg-red-50 border-l-4 border-red-500 p-3 sm:p-4 rounded-lg flex items-center gap-2 sm:gap-3 text-sm sm:text-base">
            ...
            <X className="cursor-pointer flex-shrink-0" onClick={() => setError("")} size={18} />
          </div>
        )}
            ...
            >
              ...
              <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                ...
                <div className="bg-red-50 border-l-4 border-red-600 p-3 sm:p-4 rounded-lg sm:rounded-xl">
                  <div className="flex gap-2 sm:gap-3">
                    ...
                    <div className="text-xs sm:text-sm text-red-800">
                      <strong>{t("importantLegalNotice")}</strong><br />
                      {t("legalText")}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 border rounded-lg sm:rounded-xl p-3 sm:p-4 text-xs sm:text-sm">
                  <h3 className="font-semibold mb-2 sm:mb-3">{t("complaintSummary")}</h3>
                  <div className="space-y-1.5 sm:space-y-2">
                    <div className="flex flex-col sm:flex-row sm:justify-between gap-1">
                      <span className="text-gray-600">{t("departmentLabel")}</span>
                      <span className="font-medium">{getDepartmentName(department)}</span>
                    </div>
                    <div className="flex flex-col sm:flex-row sm:justify-between gap-1">
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between gap-1">
                      ...
                      <span className="font-medium">{images.length} {t("attached")}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3 sm:space-y-4">
                  <h3 className="font-semibold text-sm sm:text-base text-gray-800">{t("iHerebyDeclare")}</h3>

                  <label className="flex items-start gap-2 sm:gap-3 cursor-pointer select-none">
                    <input
                      type="checkbox"
                      checked={verificationChecks.confirmImages}
                      onChange={e => setVerificationChecks(p => ({ ...p, confirmImages: e.target.checked }))}
                      className="mt-1 w-4 h-4 sm:w-5 sm:h-5 text-red-600 rounded border-gray-300 focus:ring-red-500 flex-shrink-0"
                    />
                    <span className="text-xs sm:text-sm">
                      <strong>{t("confirmImages")}</strong><br />
                      <span className="text-gray-600 text-[10px] sm:text-xs">{t("confirmImagesDesc")}</span>
                    </span>
                  </label>
                  ...
                  <label className="flex items-start gap-2 sm:gap-3 cursor-pointer select-none">
                    <input
                      ...
                      onChange={e => setVerificationChecks(p => ({ ...p, confirmLocation: e.target.checked }))}
                      className="mt-1 w-4 h-4 sm:w-5 sm:h-5 text-red-600 rounded border-gray-300 focus:ring-red-500 flex-shrink-0"
                    />
                    <span className="text-xs sm:text-sm">
                      <strong>{t("confirmLocation")}</strong><br />
                  ...
                  <label className="flex items-start gap-2 sm:gap-3 cursor-pointer select-none border-t pt-3 sm:pt-4">
                    <input
                      ...
                      onChange={e => setVerificationChecks(p => ({ ...p, acceptTerms: e.target.checked }))}
                      className="mt-1 w-4 h-4 sm:w-5 sm:h-5 text-red-600 rounded border-gray-300 focus:ring-red-500 flex-shrink-0"
                    />
                    <span className="text-xs sm:text-sm">
                      <strong className="text-red-700 block text-sm sm:text-base">{t("acceptTerms")}</strong>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-6 space-y-4 sm:space-y-6 border">
          {/* Department */}
          <div>
            ...
            {department && !cameraAllowed && (
              <div className="mt-3 sm:mt-4 p-3 sm:p-4 bg-blue-50 border border-blue-200 rounded-lg sm:rounded-xl">
                <p className="font-medium text-blue-800 text-sm sm:text-base">
                  {getDepartmentName(department)} {t("department")} {language === "en" ? "selected" : "தேர்ந்தெடுக்கப்பட்டது"}
                </p>
                <button
                  type="button"
            ...
            {validationMessage && (
              <p className={`mt-1.5 sm:mt-2 text-xs sm:text-sm ${cameraAllowed ? "text-green-600" : "text-amber-600"}`}>
                {validationMessage}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
              ...
              >
                <Camera className={cameraAllowed ? "text-green-600" : "text-gray-400"} size={24} />
              ...
              <button
                ...
                disabled={!cameraAllowed || images.length >= (getDepartmentDetails(department)?.maxImages || 4) || isProcessingUpload}
                className={`aspect-square border-2 border-dashed rounded-lg sm:rounded-xl flex flex-col items-center justify-center p-2
              ...
              >
                {isProcessingUpload ? (
                  ...
                  <Upload className={cameraAllowed ? "text-blue-600" : "text-gray-400"} size={24} />
                )}
                <span className="text-[10px] sm:text-xs mt-1 font-medium text-center">
              ...
              {/* Image previews */}
              {images.map((img, idx) => (
                <div key={idx} className="relative aspect-square rounded-lg sm:rounded-xl overflow-hidden border">
              ...
              <div className="flex bg-gray-100 rounded-lg p-1 text-xs sm:text-sm w-full sm:w-auto">
                <button
                  ...
                  className={`flex-1 sm:flex-none px-2 sm:px-4 py-1.5 rounded-md transition ${
                    descView === "english" ? "bg-white shadow font-medium" : "text-gray-600 hover:bg-gray-200"
                  }`}
                >
                  {t("english")}
                </button>
                <button
                  ...
                  className={`flex-1 sm:flex-none px-2 sm:px-4 py-1.5 rounded-md transition ${
                    descView === "tamil" ? "bg-white shadow font-medium" : "text-gray-600 hover:bg-gray-200"
                  }`}
                >
                  {t("tamil")}
                </button>
                <button
                  ...
                  className={`flex-1 sm:flex-none px-2 sm:px-4 py-1.5 rounded-md transition ${
                    descView === "both" ? "bg-white shadow font-medium" : "text-gray-600 hover:bg-gray-200"
                  }`}
                >
                  {t("both")}
                </button>
            ...
            <div className={`grid gap-4 sm:gap-6 ${descView === "both" ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1"}`}>
              {(descView === "english" || descView === "both") && (
                <div>
                  ...
                  <textarea
                    value={descEn}
                    onChange={e => { setDescEn(e.target.value); setTamilEdited(false); }}
                    placeholder={t("describeIssue")}
                  ...
                  />
                  {isTranslating && <p className="text-green-600 text-[10px] sm:text-xs mt-1">{t("translating")}</p>}
                </div>
              )}
              ...
              {(descView === "tamil" || descView === "both") && (
                <div>
                  ...
                  <textarea
                    value={descTa}
                    onChange={e => { setDescTa(e.target.value); setTamilEdited(true); }}
                    placeholder={t("describeIssue")}
                    rows={descView === "both" ? 4 : 5}
                    className="w-full border rounded-lg sm:rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base resize-y focus:ring-2 focus:ring-green-500"
                  />
