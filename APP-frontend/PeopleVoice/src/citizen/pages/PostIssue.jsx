...
const TEXTS = {
  en: {
    ...
    voiceInput: "Voice Input (English)",
    listening: "Listening...",
  ...
  ta: {
    ...
    voiceInput: "குரல் உள்ளீடு (ஆங்கிலம்)",
    listening: "கேட்கிறது...",
...
const PostIssue = () => {
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognition.onresult = (event) => {
        ...
        // Append to existing description with space
        setDescEn((prev) => {
  ...
  const saveDraft = useCallback(() => {
    const draft = {
      ...
      descTa,
      hashtags,
      images: images.map(img => ({ data: img.data, hash: img.hash, timestamp: img.timestamp })), // store image data for draft
      agree,
      cameraAllowed,
      descView,
    };
  ...
  const loadDraft = () => {
    ...
    if (draftJson) {
      try {
        ...
        setOtherDetail(draft.otherDetail || "");
        setReason(draft.reason || "");
        setDescEn(draft.descEn || "");
        setDescTa(draft.descTa || "");
        setHashtags(draft.hashtags || "#peoplevoice ");
        setImages(draft.images || []);
        setAgree(draft.agree || false);
        setCameraAllowed(draft.cameraAllowed || false);
        setDescView(draft.descView || "both");
        setDraftMessage(t("draftSaved"));
        setTimeout(() => setDraftMessage(""), 2000);
      } catch (e) {
  ...
  const clearDraft = () => {
    ...
    setDraftMessage(t("draftCleared"));
    setTimeout(() => setDraftMessage(""), 2000);
  };
    ...
    >
      <div className="w-full max-w-[700px] mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 pb-24 sm:pb-32">
        ...
        >
          ...
          {/* DESCRIPTION (with auto-translation and voice input) */}
          <div>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-2">
              <label
                className={`block text-xs sm:text-sm font-bold ${
            ...
            >
              {(descView === "english" || descView === "both") && (
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label
                      className={`block text-[10px] sm:text-xs font-medium ${
                        isDark ? "text-gray-400" : "text-gray-600"
                      }`}
                    >
          ...
          {/* ACTION BUTTONS: Save Draft, Load Draft, Clear Draft */}
          ...
          <button
            ...
            disabled={
              isSubmitting ||
              !cameraAllowed ||
              images.length < (getDepartmentDetails(department)?.minImages || 1)
            }
