...
const Issues = () => {
  ...
  const getActionButton = (issue) => {
    ...
    if (isOpened) {
      return {
        text: "Opened",
        color: "from-indigo-500 to-indigo-700 hover:from-indigo-600 hover:to-indigo-800",
    ...
    switch (issue.status) {
      case "In Progress":
        return { text: "In Progress", color: "from-amber-500 to-amber-700", icon: <Clock size={16} />, blink: false };
      case "Resolved":
      case "solved":
      case "Closed":
        return { text: "View Details", color: "from-blue-500 to-blue-700", icon: <Eye size={16} />, blink: false };
      default:
        return { text: "View Issue", color: "from-purple-500 to-purple-700", icon: <FileText size={16} />, blink: false };
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-blue-50/20 p-6 md:p-10">
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="mb-8 flex flex-wrap justify-between items-center gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-gray-600 font-medium">{adminDistrict} Municipality</span>
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
              <span className="text-emerald-600 text-sm font-medium">Live Updates (8s)</span>
            </div>
          </div>
          <button
            onClick={() => { fetchIssues(true); fetchGlobalStats(); }}
          ...
          >
            <RefreshCw size={16} className="group-hover:rotate-180 transition-transform" /> Manual Refresh
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
          {[
            { label: "New Reports", val: globalStats.new, icon: AlertCircle, gradient: "from-blue-500 to-blue-600", bg: "from-blue-50 to-blue-100/50" },
            { label: "In Progress", val: globalStats.ongoing, icon: Clock, gradient: "from-amber-500 to-amber-600", bg: "from-amber-50 to-amber-100/50" },
            { label: "Resolved", val: globalStats.resolved, icon: CheckCircle, gradient: "from-emerald-500 to-emerald-600", bg: "from-emerald-50 to-emerald-100/50" },
            { label: "Closed", val: globalStats.closed, icon: Archive, gradient: "from-gray-500 to-gray-600", bg: "from-gray-50 to-gray-100/50" },
          ].map((s, i) => (
            <div
              key={i}
            ...
            >
              <div className="flex justify-between items-start">
                ...
                <div className={`p-3 rounded-full bg-gradient-to-br ${s.gradient} shadow-lg group-hover:scale-110 transition-transform`}>
          ...
          <>
                ...
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredIssues.map((issue, idx) => {
                      ...
                      return (
                        <tr key={issue._id} className="hover:bg-white/50 transition-all duration-200 group">
                          <td className="px-6 py-4 text-sm font-medium text-gray-500">{(page - 1) * limit + idx + 1}</td>
                          ...
                          <td className="px-6 py-4 text-sm text-gray-600 font-medium">{issue.area || "Not specified"}</td>
                          <td className="px-6 py-4 text-sm text-gray-500">{new Date(issue.createdAt).toLocaleDateString()}</td>
                          <td className="px-6 py-4">
                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border backdrop-blur-sm ${statusDisplay.color}`}>
            ...
            {totalPages > 1 && (
              <div className="flex justify-between items-center mt-6 bg-white/50 backdrop-blur-sm p-3 rounded-xl border border-white/50">
                <div className="text-sm text-gray-500 font-medium">Page {page} of {totalPages}</div>
